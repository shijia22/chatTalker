      ...
      <div class="row justify-center">
        <div class="col lg:flex lg:flex-row-reverse lg:items-center">
          ...
          <ul class="mx-[3.75rem] lg:mr-[8.625rem] lg:ml-0 lg:grid lg:grid-cols-2 lg:gap-12">
            <li v-for="person in people" :key="person.name" class="mb-6 pt-8 px-6 pb-4 overflow-hidden rounded-xl shadow-card">
              <div class="w-[85px] mb-6 p-4 rounded-full shadow-card"><img :src="person.icon" :alt="person.alt" class="w-full"></div>
              <div class="flex flex-col min-h-[128px]">
                <span class="mb-4 font-bold text-h3 leading-6">{{ person.name }}</span>
                <ul class="flex flex-col rounded-b-2xl">
